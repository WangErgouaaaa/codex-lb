...
- Permit a fresh cross-account replay for a durable count-and-fingerprint-verified full resend whose deterministic plaintext projection is account-neutral and self-contained. When a stuck timeout has cleared the response anchor and its prefix metadata, also permit an anchorless Codex resend only when a hard session header still resolves a durable account owner and the complete client-carried history projects to an explicitly safe plaintext request. Remove owner-bound reasoning, upstream item identities, and completed search bookkeeping before validation; shape-validate retained internal turn metadata; then remove the previous-response anchor and stale session aliases, exclude the failed owner, and create a local server-namespaced recovery lane.
