...
@pytest.mark.asyncio
async def test_checkpoint_pointer_survives_anchor_clear(coordinator: DurableBridgeSessionCoordinator) -> None:
    """The canonical checkpoint pointer is deliberately owner-fence-free and
    outlives ``clear_latest_response_anchor`` so an owner-unavailable
    continuation can still be rehydrated after a stuck-timeout anchor clear."""
    session_id = await _claim_session(coordinator)
    await coordinator.save_checkpoint(
    ...
    await coordinator.set_session_checkpoint_pointer(session_id=session_id, response_id="resp_completed")
    lookup = await coordinator.lookup_request_targets(
        session_key_kind="session_header",
        session_key_value="sid-cp",
        api_key_id="key-cp",
        turn_state=None,
        session_header="sid-cp",
        previous_response_id=None,
    )
    assert lookup is not None
    assert lookup.latest_checkpoint_response_id == "resp_completed"
...
def test_checkpoint_models_exist() -> None:
    ...
    assert "latest_checkpoint_response_id" in HttpBridgeSessionRecord.__table__.columns
